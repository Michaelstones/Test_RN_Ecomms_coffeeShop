...
import {navigateToTabScreen} from '../lib/depNavigation';
import useAddToCart from '../lib/addToCartNetwork';
...
const PaymentScreen = ({navigation, route}: any) => {
  ...
  const addToOrderHistory = useStore((state: any) => state?.addToOrderHistoryListFromCart);
  const userId = useStore((state: any) => state?.user)?.id;
  const productId = useStore((state: any) => state?.OrderHistoryList)[0]?.cart[0].id;
  ...
  const buttonPressHandler = async () => {
    try {
      ...
      await addToCart({
        userId,
        productId,
        quantity: 1, // TODO: get quantity from cart but for now we have only one item
      });
      setTimeout(() => {
        setShowAnimation(false);
        navigateToTabScreen(navigation, 'Tab', 'OrderHist');
      }, 2000);
    } catch (error) {
      setToastMessage('An error occurred while updating orderkist.');
      setShowToast(true);
    }
  };
  return (
    <View>
      ...
      {showAnimation ? (
        <PopAnime style={styles.LottieAnimation} source={require('../lottie/successful.json')} />
      ) : (
        <></>
      )}
        ...
        <View style={styles.PaymentOptionsContainer}>
              ...
              <View style={styles.CreditCardBG}>
                <LinearGradient
                  ...
                  <View style={styles.CreditCardRow}>
                    <CustomIconConfig name="chip" size={FONTSIZE.size_20 * 2} color={COLORS.primaryOrangeHex} />
                    <CustomIconConfig name="visa" size={FONTSIZE.size_30 * 2} color={COLORS.primaryWhiteHex} />
                  </View>
          ...
          {PaymentList?.map((data: any) => (
            <TouchableOpacity
              ...
              <PayMethod paymentMode={paymentMode} name={data.name} icon={data.icon} isIcon={data.isIcon} />
            </TouchableOpacity>
          ))}
        </View>
      </ScrollView>
